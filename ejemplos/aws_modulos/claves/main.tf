...
resource "tls_private_key" "clave_privada" {
    ...
    provisioner "local-exec" {
        command = "echo \"${self.private_key_pem}\" > ${var.id_clave}_pri.pem && echo \"${self.public_key_pem}\" > ${var.id_clave}_pub.pem"
    }
    
    provisioner "local-exec" {
        command = "chmod 700 ${var.id_clave}_pri.pem && chmod 700 ${var.id_clave}_pub.pem"
    }
}
